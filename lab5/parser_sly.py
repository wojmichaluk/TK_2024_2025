...
class Mparser(Parser):
    ...
    @_('FOR ID "=" id_int ":" int_expr statement')
    ...
    @_('print_recursive "," print_expr')
    def print_recursive(self, p):
        return AST.PrintRecursive(p[0], p[2], p.lineno)

    @_('print_expr')
    def print_recursive(self, p):
        return AST.PrintExpr(p[0], p.lineno)
    
    @_('value')
    def print_expr(self, p):
    ...
    @_('expression "+" expression',
       ...
       'expression "/" expression')
    def print_expr(self, p):
        return AST.ArithNumExpr(p[1], p[0], p[2], p.lineno)
    ...
    @_('expression "\'"',
       '"(" expression ")"')
    def expression(self, p):
        special_op = p[1] if len(p) == 2 else f"{p[0]}{p[2]}"
        return AST.GeneralExpression(p[-2], special_op, p.lineno)
    ...
    def expression(self, p):
        return AST.ArithMatExpr(p[1], p[0], p[2], p.lineno)
    ...
    def assign_expression(self, p):
        return AST.UpdateExpr(p[1], p[0], p[2], p.lineno)
    ...
    def relation_expression(self, p):
        return AST.CompExpr(p[1], p[0], p[2], p.lineno)
